...
CREATE PROCEDURE ENCRYPTPASSWORD(IN PASSWORD VARCHAR
(255), OUT ENCRYPTED_PASSWORD VARBINARY(255)) BEGIN 
...
CREATE PROCEDURE DECRYPTPASSWORD(IN ENCRYPTED_PASSWORD 
VARBINARY(255), OUT DECRYPTED_PASSWORD VARCHAR(255
)) BEGIN 
	DECLARE key_var CHAR(16);
	SET key_var = 'arsac';
	SET
	    decrypted_password = CAST(
	        AES_DECRYPT(encrypted_password, key_var) AS CHAR
	    );
	END 
$ 

CREATE PROCEDURE INSERTAR_CUENTA_USUARIO(OUT _ID_CUENTA_USUARIO 
INT, IN _FID_EMPLEADO INT, IN _USUARIO VARCHAR(50)
, IN _CONTRASENA VARCHAR(50)) BEGIN 
...
CREATE PROCEDURE VERIFICAR_CUENTA_USUARIO(_USUARIO 
VARCHAR(50), _CONTRASENA VARCHAR(50)) BEGIN 
	...
	WHERE
	    usuario = _usuario
	    AND contrasena = _encrypted_password
	    AND activo = 1;
END$ 

-- EMPLEADO

CREATE PROCEDURE LISTAR_TIPOS_DE_EMPLEADOS() BEGIN 
	SELECT
	    id_tipo_empleado,
	    descripcion
	from tipoEmpleado
...
INT, IN _DNI VARCHAR(8), IN _NOMBRE VARCHAR(70), IN 
_APELLIDOS VARCHAR(70), IN _CORREO VARCHAR(70), IN 
_TELEFONO VARCHAR(70), IN _FID_SEDE INT, IN _FID_TIPO_EMPLEADO 
INT, IN _FECHA_CONTRATACION DATE, IN _SALARIO DECIMAL
(10, 2), IN _DIRECCION VARCHAR(100)) BEGIN 
	INSERT INTO
	    persona(
	        nombre,
	        apellidos,
...
(IN _ID_SEDE INT, IN _NOMBRE_DNI VARCHAR(1000)) BEGIN 
	SELECT
	    e.id_empleado,
	    p.nombre,
	    p.apellidos,
	...
	where (
	        CONCAT(p.nombre, ' ', p.apellidos) LIKE CONCAT('%', _nombre_DNI, '%')
	    )
	    OR (
	        p.DNI LIKE CONCAT('%', _nombre_DNI, '%')
	    );
	END 
$ 

CREATE PROCEDURE LISTAR_EMPLEADOS_POR_NOMBRE_DNI(IN 
_NOMBRE_DNI VARCHAR(255)) BEGIN 
	SELECT
	    e.id_empleado,
	    p.nombre,
	    p.apellidos,
...
CREATE PROCEDURE BUSCAR_EMPLEADO_ID(IN _IDEMPLEADO 
INT) BEGIN 
	SELECT
	    e.id_empleado,
	    p.nombre,
	...
	where
	    _idEmpleado = id_empleado;
	END 
$ 

CREATE PROCEDURE ACTUALIZAR_EMPLEADO(IN _IDEMPLEADO 
INT, IN _DNI VARCHAR(8), IN _NOMBRE VARCHAR(70), IN 
_APELLIDOS VARCHAR(70), IN _CORREO VARCHAR(70), IN 
_TELEFONO VARCHAR(70), IN _FID_SEDE INT, IN _FID_TIPO_EMPLEADO 
INT, IN _FECHA_CONTRATACION DATE, IN _SALARIO DECIMAL
(10, 2), IN _DIRECCION VARCHAR(100)) BEGIN 
	...
	SET
	    ...
	    telefono = _telefono
	where
	    id_persona = _idEmpleado;
	UPDATE empleado
	SET
	    fid_sede = _fid_sede,
	    fid_tipo_empleado = _fid_tipo_empleado,
...
CREATE PROCEDURE ELIMINAR_EMPLEADO(IN _ID_EMPLEADO 
INT) BEGIN 
	UPDATE persona
	SET activo = false
	where
	    id_persona = _id_empleado;
	END 
$ 
...
INT, IN _DNI VARCHAR(8), IN _NOMBRE VARCHAR(70), IN 
_APELLIDOS VARCHAR(70), IN _CORREO VARCHAR(70), IN 
_TELEFONO VARCHAR(70), IN _RUC VARCHAR(50), IN _RAZON_SOCIAL 
VARCHAR(50)) BEGIN 
	INSERT INTO
	    persona(
	        nombre,
	...
	INSERT INTO
	    clienteMayorista(RUC, razon_social)
	VALUES
	(_RUC, _razon_social);
	END 
$ 
...
(IN _NOMBRE_DNI VARCHAR(1000)) BEGIN 
	SELECT
	    c.id_cliente_mayorista,
	    p.nombre,
	    p.apellidos,
	    p.DNI,
	    p.correo,
...
CREATE PROCEDURE ACTUALIZAR_CLIENTE_MAYORISTA(IN _ID_CLIENTE_MAYORISTA 
INT, IN _DNI VARCHAR(8), IN _NOMBRE VARCHAR(70), IN 
_APELLIDOS VARCHAR(70), IN _CORREO VARCHAR(70), IN 
_TELEFONO VARCHAR(70), IN _RUC VARCHAR(50), IN _RAZON_SOCIAL 
VARCHAR(50)) BEGIN 
	...
	SET
	    ...
	    telefono = _telefono
	where
	    id_persona = _id_cliente_mayorista;
	UPDATE clienteMayorista
	SET
	    RUC = _RUC,
	    razon_social = _razon_social
	WHERE
	    id_cliente_mayorista = _id_cliente_mayorista;
	END 
$ 

CREATE PROCEDURE ELIMNAR_CLIENTE_MAYORISTA(IN _ID_CLIENTE_MAYORISTA 
INT) BEGIN 
	UPDATE persona
	SET activo = false
	where
